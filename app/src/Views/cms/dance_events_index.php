    ...
    <main class="mx-auto max-w-6xl p-4 py-8">
        <section class="rounded-2xl bg-white p-6 shadow-xl ring-1 ring-slate-200">
            <div class="flex items-center justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Dance Events</h1>
                    <p class="mt-1 text-sm text-slate-600">Create, edit, and delete dance events securely.</p>
                </div>

                <div class="flex items-center gap-3">
                    <a href="/cms/events/dance/create"
            ...
            <div class="mt-6 overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        <?php if (empty($events)): ?>
                            <tr>
                                <td colspan="7" class="px-4 py-6 text-slate-600">No dance events found.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($events as $e): ?>
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <?php
                                    $artistName = trim((string)($e->artist_name ?? ''));
                                    if ($artistName === '') {
                                        $title = (string)($e->title ?? '');
                                        $artistName = trim((string)preg_replace('/\s*\([^)]*\)\s*$/', '', $title));
                                    }
                                    ?>
                                    <td class="px-4 py-3 text-slate-700"><?= htmlspecialchars($artistName !== '' ? $artistName : '-') ?></td>
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3">
                                        <div class="flex flex-wrap gap-2">
                                            <a href="/cms/events/dance/<?= (int)$e->event_id ?>"
                                               class="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700">
                                                Edit dance event
                                            </a>

                                            <form method="POST"
                                                  action="/cms/events/dance/<?= (int)$e->event_id ?>/delete"
                                                  onsubmit="return confirm('Delete this dance event? This cannot be undone.');">
