    ...
    <main class="mx-auto max-w-5xl p-4 py-8">
        <section class="rounded-2xl bg-white p-6 shadow-xl ring-1 ring-slate-200">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Edit Dance Event</h1>
                ...
                <a href="/cms/events/dance" class="rounded-lg bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-200">
                    ← Back to Dance Events
                </a>
            </div>
            ...
            <form method="POST" enctype="multipart/form-data" action="/cms/events/dance/<?= (int)($event->event_id ?? 0) ?>" class="mt-6 space-y-8">
                ...
                <div>
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div class="sm:col-span-2">
                            ...
                            <input name="title" type="text" required value="<?= htmlspecialchars((string)($event->title ?? '')) ?>"
                ...
                <div class="border-t border-slate-200 pt-6">
                    <h2 class="text-lg font-semibold text-slate-900">Dance details</h2>
                    <p class="mt-1 text-sm text-slate-600">Stored in the <span class="font-medium">DanceEvent</span> table.</p>
                    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                        ...
                        <div>
                            ...
                            <input name="end_date" type="datetime-local" value="<?= htmlspecialchars((string)toDatetimeLocal($event->end_date ?? null)) ?>"
                                class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200">
                        </div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700">Artist (optional)</label>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input name="img_background_file" type="file" accept=".jpg,.jpeg,.png,.webp,.gif"
                                class="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm file:mr-4 file:rounded-lg file:border-0 file:bg-slate-100 file:px-3 file:py-2 file:text-sm file:font-semibold file:text-slate-700 hover:file:bg-slate-200">
                        ...
                        <div>
                            ...
                            <input name="price" type="number" step="0.01" min="0" value="<?= htmlspecialchars((string)($event->price ?? '')) ?>"
                                class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200">
                        </div>
                        <div>
                ...
                <div class="flex items-center justify-end gap-3 border-t border-slate-200 pt-6">
                    <a href="/cms/events/dance" class="rounded-xl bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-200">Cancel</a>
