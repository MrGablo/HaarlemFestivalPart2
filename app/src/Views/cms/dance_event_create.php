...
<head>
    ...
    <title>Create Dance Event</title>
    ...
    <main class="mx-auto max-w-5xl p-4 py-8">
        <section class="rounded-2xl bg-white p-6 shadow-xl ring-1 ring-slate-200">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Create Dance Event</h1>
                    <p class="mt-1 text-sm text-slate-600">Add a new dance event with parent and dance-specific details.</p>
                </div>

                <a href="/cms/events/dance"
            ...
            <form method="POST"
                ...
                action="/cms/events/dance/create"
                ...
                <div>
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div class="sm:col-span-2">
                            ...
                            <input name="title" type="text" maxlength="120" required value="<?= $v('title') ?>"
                ...
                <div class="border-t border-slate-200 pt-6">
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input name="start_date" type="datetime-local" required value="<?= $v('start_date') ?>"
                                class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-700">End date</label>
                            <input name="end_date" type="datetime-local" required value="<?= $v('end_date') ?>"
                                class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200">
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input name="img_background_file" type="file" accept=".jpg,.jpeg,.png,.webp,.gif"
                                class="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm
                                       file:mr-4 file:rounded-lg file:border-0 file:bg-slate-100 file:px-3 file:py-2 file:text-sm file:font-semibold file:text-slate-700 hover:file:bg-slate-200">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-700">Price</label>
                            <input name="price" type="number" step="0.01" min="0" required value="<?= $v('price', '0.00') ?>"
                                class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200">
                        </div>
