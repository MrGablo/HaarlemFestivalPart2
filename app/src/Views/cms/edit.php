    ...
    <main class="mx-auto max-w-5xl p-4 py-8">
        <section class="rounded-2xl bg-white p-6 shadow-xl ring-1 ring-slate-200">
            <div class="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-600">
                        #<?= (int)($page['Page_ID'] ?? 0) ?> · <?= CmsForm::h((string)($page['Page_Title'] ?? '')) ?> · <?= CmsForm::h((string)($page['Page_Type'] ?? '')) ?>
                    </p>
                </div>
            ...
            <form method="POST" action="/cms/page/<?= (int)($page['Page_ID'] ?? 0) ?>/update" class="mt-6 space-y-4">
                <?php if ($content === []): ?>
                    <p class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">This page currently has empty JSON content.</p>
                <?php else: ?>
                ...
                <div class="flex items-center gap-3 pt-2">
                    <button type="submit" class="rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-300">
                        Save content
                    </button>
