    ...
    <main class="mx-auto max-w-5xl p-4 py-8">
        <section class="rounded-2xl bg-white p-6 shadow-xl ring-1 ring-slate-200">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Export Orders</h1>
                    <p class="mt-1 text-sm text-slate-600">Choose columns for <?= htmlspecialchars((string)$request->scope) ?> scope export.</p>
                </div>
            ...
            <form method="GET" action="/cms/orders/export" class="mt-6 space-y-6">
                <input type="hidden" name="search" value="<?= htmlspecialchars((string)$request->criteria->search) ?>">
                <input type="hidden" name="status" value="<?= htmlspecialchars((string)$request->criteria->statusFilter) ?>">
                <input type="hidden" name="sort" value="<?= htmlspecialchars((string)$request->criteria->sortColumn) ?>">
                <input type="hidden" name="dir" value="<?= htmlspecialchars((string)$request->criteria->sortDirection) ?>">
                <input type="hidden" name="scope" value="<?= htmlspecialchars((string)$request->scope) ?>">
                <?php if ($request->userId > 0): ?>
                    <input type="hidden" name="user_id" value="<?= (int)$request->userId ?>">
                <?php endif; ?>
                ...
                <div class="max-w-sm">
                    ...
                    <select name="format"
                        ...
                        <option value="excel" <?= $request->format === 'excel' ? 'selected' : '' ?>>Excel (XML)</option>
                    </select>
                </div>

                <div>
                    <h2 class="text-base font-semibold text-slate-900">Columns for export</h2>
                    <p class="mt-1 text-sm text-slate-600">Uncheck fields you do not want to include.</p>
