...
<section id="schedule" class="schedule">
  ...
  <div class="schedule__filters">
    <div class="schedule__tabs">
      <span class="schedule__label"><?= htmlspecialchars($filters['group_label'] ?? 'By date') ?></span>

      <?php foreach ($hallTabs as $i => $tab): ?>
        <button
          type="button"
          class="chip hall-chip <?= $i === 0 ? 'is-active' : '' ?>"
          data-hall="<?= htmlspecialchars($tab) ?>"
    ...
    <div class="schedule__days">
      <?php foreach ($days as $i => $d): ?>
        <button
          type="button"
          class="chip day-chip <?= $i === 0 ? 'is-active' : '' ?>"
          data-day="<?= htmlspecialchars($d) ?>"
        ><?= htmlspecialchars($d) ?></button>
      <?php endforeach; ?>
    </div>
  </div>
  ...
  <div class="event-grid" id="eventGrid">
    <?php foreach ($events as $ev): ?>
      <article
        class="event-card"
        data-hall="<?= htmlspecialchars($ev['hall']) ?>"
      ...
      >
        <a class="event-card__media"
          ...
          <div class="event-card__overlay">
            ...
            <div class="event-card__meta"><?= htmlspecialchars($ev['display_date']) ?> <?= htmlspecialchars($ev['display_time']) ?></div>
          </div>
        </a>
  ...
  <div class="schedule__footer">
    ...
    <?php if (is_array($allBtn) && !empty($allBtn['href'])): ?>
      <a class="all-events" href="<?= htmlspecialchars((string)$allBtn['href']) ?>">
