...
# Install system dependencies, Composer and Microsoft SQL drivers
RUN set -eux; \
    apt-get update; \
    apt-get install -y --no-install-recommends \
        git unzip libzip-dev gnupg apt-transport-https curl build-essential unixodbc-dev ca-certificates; \
